...
import { criarProdutoMasterAction } from "@/app/actions/masterProducts";
...
export default function NewMasterProductForm() {
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState("");
    const [success, setSuccess] = useState<{ id: number; name: string; type: string; apiKey: string } | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Server action: quem autoriza é a sessão de admin. A chave do painel
        // master fica no servidor — antes vinha escrita aqui, dentro do pacote
        // que o navegador de qualquer visitante baixa.
        try {
            const fd = new FormData();
            fd.set("name", formData.name);
            fd.set("type", formData.type);
            fd.set("description", formData.description);
            fd.set("packageName", formData.packageName);
            fd.set("webhookUrl", formData.webhookUrl);

            const r = await criarProdutoMasterAction(fd);
            if (r.error) setError(r.error);
            else if (r.product) setSuccess(r.product);
        } catch {
