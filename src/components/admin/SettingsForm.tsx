...
// "Diária" saiu: pagar por dia não é uma conta por corrida e, do jeito que
// estava, toda entrega fechava com R$ 0,00 na carteira do motoboy.
const PAYMENT_MODELS = [
    { id: "fixed", label: "Taxa Fixa", description: "Valor único por entrega realizada." },
    { id: "distance", label: "Por KM", description: "Valor por quilômetro da loja até o cliente." },
...
export default function SettingsForm({ initialData }: SettingsFormProps) {
    ...
    // Loja que tinha "Diária" salva passa a valer como Taxa Fixa (com aviso na tela).
    const tinhaDiaria = initialData?.remunerationModel === "daily";
    const [model, setModel] = useState<"fixed" | "distance" | "hybrid">(
        tinhaDiaria ? "fixed" : ((initialData?.remunerationModel as "fixed" | "distance" | "hybrid") || "fixed")
    );
                ...
                {tinhaDiaria && (
                    <div className="bg-amber-600/20 border border-amber-600/40 rounded-xl p-4 mb-6">
                        <p className="text-amber-200 text-sm">
                            A opção <strong>Diária</strong> saiu do sistema — ela não calculava nada e as corridas
                            fechavam com R$ 0,00 pro motoboy. Sua loja está valendo como <strong>Taxa Fixa</strong>.
                            Confira o valor abaixo e salve.
                        </p>
                    </div>
                )}
                ...
                <div className="space-y-4">
                    ...
                    <div className="pt-4 border-t border-zinc-700">
                        <label className="flex items-center gap-2 text-sm font-medium text-zinc-300 mb-1">
                            Mínimo por corrida (Opcional)
                            <div title="Piso: se a conta der menos que isso, a corrida paga esse valor." className="text-zinc-500 cursor-help">
                                <HelpCircle size={14} />
                            </div>
                        </label>
                        <div className="relative">
                        ...
                        <p className="text-xs text-zinc-500 mt-1">Piso de cada entrega: se o cálculo der menos que isso, vale esse valor. Deixe 0,00 pra não usar.</p>
                    </div>
                </div>
            </div>
